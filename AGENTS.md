...
- Release control: GitHub default/source branch is `main`; Vercel Git deployment and Railway autodeploy remain disabled. Vercel production deployment `FVQiW8qX8533PaXSGNpV3qqLkScb` and Railway deployment `e97087a8-6abd-4b74-a817-b5508ca84baf` run the approved SHA. Railway uses `/railway.worker.toml` and `/Dockerfile.worker` with a 60-second drain.
