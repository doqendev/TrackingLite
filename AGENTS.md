...
- Release control: GitHub default/source branch is `main`; Vercel Git deployment is disabled. Railway GitHub autodeploy was discovered ENABLED during the 2026-08-02 cutover: it builds the worker from `main` merges that touch watched code paths (doc-only merges did not trigger builds), and the release gate passed because `TRACKCLEAR_PRODUCTION_RELEASE_SHA` was rotated before the merge build went live. Verify or disable autodeploy before the next controlled release. Vercel production deployment `tracking-lite-ee1awfd5e` and Railway deployment `48142d27-47fc-4226-88df-05c498207eaf` run the approved SHA `20844b3c619fc1affefb5c6031fc01f1dc5b648e`. Railway uses `/railway.worker.toml` and `/Dockerfile.worker` with a 60-second drain.
