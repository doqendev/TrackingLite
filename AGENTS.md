...
**Enums:** Platform, WorkspaceProductMode, WorkspaceInstallType, CatalogIdMode, EventName (6 events including Refund), EventStatus (PENDING/SENT/FAILED/RETRYING/SUPERSEDED), ConsentMode, BillingPlan, SubscriptionStatus, Destination (7 destinations), ShopifyWebhookInboxStatus (PENDING/PROCESSING/PROCESSED/EXPIRED)
...
- **Deployment order:** Pin the verified production database name/schema/system identifier, apply the eleven branch migrations and verify eleven indexes, stop/drain all old workers, deploy the new web build while delivery is paused, wait for zero old Vercel invocations plus the configured duration buffer, then start exactly one new worker fleet. Never run mixed old/new workers or use a prebuilt production Vercel deployment. After candidate traffic, the raw baseline worker is not a safe rollback. See `docs/deploy.md`.
