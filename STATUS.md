...
- Cutover deviations, both benign here but material for the next release: (1) `railway variables --set` triggers a redeploy unless `--skip-deploys` is passed. Rotating the release pin therefore rebuilt the worker from the connected `main` branch and brought it live about two minutes before the new web deployment. The gate passed because that rebuild used the exact SHA the pin was being set to. Treat the pin rotation as an explicit deploy step, or pass `--skip-deploys` when a variable change must not deploy. (2) Two `railway up` CLI deployments (`128f8afd`, `c836cfbd`) failed without going live, consistent with the release gate failing closed on CLI uploads that carry no git commit metadata; a git-sourced Railway deployment is the working path for the worker.
- Correction: an earlier revision of this file, `AGENTS.md`, and `CLAUDE.md` claimed Railway GitHub autodeploy was found enabled during this cutover. That was wrong. The dashboard shows auto deploy disabled for `TrackingLite-Worker`; the deployment was triggered by the variable rotation described above. Do not re-introduce the autodeploy claim.
