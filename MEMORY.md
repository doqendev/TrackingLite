...
- Existing Dirava order `#5076`/`#5077` evidence remains the regression baseline. Repasting `bridge-v1`, buy-now/direct checkout, returning visitor, delayed checkout, live non-default catalog modes, Meta/TikTok UI visibility, and a controlled post-deploy paid order remain open. The current saved browser identity is Mizoke-only and cannot access Dirava Shopify admin; do not claim the repaste or paid canary passed until an authorized Dirava session is supplied.
