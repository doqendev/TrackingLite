...
## 2026-07-27 - Own-Store Tracking Hardening (Branch Only; Not Deployed)

- Work is isolated on `codex/own-store-tracking-hardening`; production, Dirava, and Mizoke configuration/store repositories were not changed.
...
- The `bridge-v1` pasted Custom Pixel loader subscribes to all seven Shopify events synchronously, keeps at most 100 early events in one ordered FIFO during remote initialization, and activates after all handlers register. Its versioned pixel URL bypasses transition cache, but existing stores must repaste the new loader; the public pixel scripts now use a 30-second shared-cache TTL.
...
- Existing Dirava evidence remains the regression baseline. PostgreSQL 16/Redis 7 PR checks, production migration/cutover, repasting `bridge-v1`, buy-now/direct checkout, returning visitor, delayed checkout, live non-default catalog modes, Meta/TikTok UI visibility, and controlled post-deploy orders remain explicit gates; no template is treated as exercised evidence.
- Production remains on `506f432`. `main` remains the source branch, but the hardening branch disables Vercel Git deployment and requires Railway autodeploy/config paths to be verified and controlled in the provider. Railway currently shows a past-due account warning, so no production rollout should begin until billing, provider configuration, database/Redis safeguards, and deployment control are confirmed.
